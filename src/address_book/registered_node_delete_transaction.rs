...
impl TransactionExecute for RegisteredNodeDeleteTransactionData {
    fn execute(
        ...
        Box::pin(async {
            AddressBookServiceClient::new(channel).delete_registered_node(request).await
        })
    }
}
...
mod tests {
    ...
    use crate::address_book::RegisteredNodeDeleteTransactionData;
    use crate::protobuf::FromProtobuf;
    ...
    fn make_transaction() -> RegisteredNodeDeleteTransaction {
        ...
        tx.registered_node_id(42).freeze().unwrap();
    ...
    #[test]
    fn from_proto_body() {
        let tx = services::RegisteredNodeDeleteTransactionBody { registered_node_id: 42 };
