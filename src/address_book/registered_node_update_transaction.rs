...
impl RegisteredNodeUpdateTransaction {
    ...
    #[must_use]
    pub fn get_service_endpoints(&self) -> &[RegisteredServiceEndpoint] {
        &self.data().service_endpoints
    }

    /// Sets the list of service endpoints.
    pub fn service_endpoints(
        &mut self,
        service_endpoints: impl IntoIterator<Item = RegisteredServiceEndpoint>,
    ) -> &mut Self {
        self.data_mut().service_endpoints = service_endpoints.into_iter().collect();
        self
    }
...
impl TransactionExecute for RegisteredNodeUpdateTransactionData {
    fn execute(
        ...
        Box::pin(async {
            AddressBookServiceClient::new(channel).update_registered_node(request).await
        })
    }
}
...
impl ToProtobuf for RegisteredNodeUpdateTransactionData {
    ...
    fn to_protobuf(&self) -> Self::Protobuf {
        services::RegisteredNodeUpdateTransactionBody {
            ...
            service_endpoint: self.service_endpoints.iter().map(|it| it.to_protobuf()).collect(),
        }
    }
}
...
mod tests {
    ...
    use crate::address_book::RegisteredNodeUpdateTransactionData;
    use crate::protobuf::{
        FromProtobuf,
        ToProtobuf,
    };
    ...
    fn from_proto_body() {
        ...
        let tx = services::RegisteredNodeUpdateTransactionBody {
            ...
            service_endpoint: endpoints.iter().map(|it| it.to_protobuf()).collect(),
        };

        let data = RegisteredNodeUpdateTransactionData::from_protobuf(tx).unwrap();

        assert_eq!(data.registered_node_id, 42);
        ...
        assert_eq!(data.service_endpoints, endpoints);
    }
    ...
    fn get_set_registered_node_id() {
        ...
        assert_eq!(tx.get_registered_node_id(), 42);
    }

    #[test]
    #[should_panic]
    fn get_set_registered_node_id_frozen_panic() {
        make_transaction().registered_node_id(42);
    }
