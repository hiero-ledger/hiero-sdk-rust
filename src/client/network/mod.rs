...
impl NetworkData {
    ...
    pub(crate) fn channel(&self, index: usize) -> (AccountId, Channel) {
        ...
        let channel = if use_tls {
            ...
            self.connections[index].channel_with_tls_conversion()
        } else {
            // When TLS is disabled, use addresses as-is (port 50211)
            self.connections[index].channel()
        };
...
impl NodeConnection {
    ...
    pub(crate) fn channel(&self) -> Channel {
        ...
        if self.uses_tls() {
            ...
            let channels = self.tls_channels.get_or_init(|| self.create_all_tls_channels());
            ...
            self.channel.get_or_init(|| self.create_plaintext_channel()).clone()
        }
    }

    /// Create channel with TLS conversion (converts port 50211 -> 50212)
    pub(crate) fn channel_with_tls_conversion(&self) -> Channel {
        ...
        tls_connection.channel()
    }

    fn create_plaintext_channel(&self) -> Channel {
    ...
    fn create_tls_channel(&self) -> Channel {
        ...
        self.create_all_tls_channels()[0].clone()
    }

    fn create_all_tls_channels(&self) -> Vec<Channel> {
