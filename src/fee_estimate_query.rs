...
impl FeeEstimate {
    ...
    pub fn subtotal(&self) -> u64 {
        self.base + self.extras.iter().map(|e| e.amount).sum::<u64>()
    }
}
...
pub struct FeeExtra {
    ...
    pub count: u64,
    /// The amount charged, in tinycents
    pub amount: u64,
...
mod tests {
    ...
    fn test_fee_estimate_subtotal() {
        let fee = FeeEstimate {
            base: 100,
            extras: vec![
                FeeExtra { count: 1, amount: 10, name: "extra1".to_string() },
                FeeExtra { count: 2, amount: 20, name: "extra2".to_string() },
            ],
        };
