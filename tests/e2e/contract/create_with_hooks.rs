...
async fn contract_create_with_hook_and_storage_updates() -> anyhow::Result<()> {
    ...
    let lambda_hook = EvmHook::new(EvmHookSpec::new(Some(contract_id)), vec![storage_update]);
...
async fn contract_create_fails_when_lambda_hook_missing_contract_id() -> anyhow::Result<()> {
    ...
    let lambda_hook = EvmHook::new(EvmHookSpec::new(None), vec![storage_update]);
    ...
    assert_matches!(result, Err(hiero_sdk::Error::ReceiptStatus { status, .. }) if status == Status::InvalidHookCreationSpec);
